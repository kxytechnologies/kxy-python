...
if __name__ == '__main__':
	...
	logging.info('Overall Classification Feasibility: %.2f nats' % df.kxy.classification_feasibility('Is Fake'))
	...
	importance_df_2 = df.kxy.incremental_input_importance('Is Fake', greedy=True)
	logging.info('\n %s' % importance_df_2.round(2))

	logging.info('')
	logging.info('')
	logging.info('Incremental Input Importance (Primal)')
	importance_df_3 = df.kxy.incremental_input_importance('Is Fake', space='primal', greedy=True)
	...
	logging.info('Suboptimality: %.6f' %test_df.kxy.classification_suboptimality('prediction', 'Is Fake', discrete_input_columns=(), \
				continuous_input_columns=()))
	# How does it compare to the best case scenario
	logging.info('Training Classification Feasibility: %.2f nats' % train_df.kxy.classification_feasibility('Is Fake'))
