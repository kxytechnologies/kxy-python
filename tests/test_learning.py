...
from kxy.learning import get_xgboost_learner, get_tensorflow_dense_learner, get_pytorch_dense_learner, get_lightgbm_learner
...
def test_lean_boosted_lightgbm_regressor():
	# Regression
	xgboost_regressor_cls = get_lightgbm_learner('lightgbm.LGBMRegressor')
	dataset = Abalone()
	target_column = dataset.y_column
	df = dataset.df
...
def test_lean_boosted_tensorflow_regressor():
	...
	tf_regressor_cls = get_tensorflow_dense_learner('KerasRegressor', layers, loss, optimizer=optimizer, \
		sk_params={'epochs': 10, 'batch_size': 100})
	dataset = Abalone()
	target_column = dataset.y_column
	df = dataset.df
...
def test_lean_boosted_pytorch_regressor():
	...
	pt_regressor_cls = get_pytorch_dense_learner('skorch.NeuralNetRegressor', layers, sk_params={'max_epochs': 10, 'batch_size': 100})
	dataset = Abalone()
	target_column = dataset.y_column
	df = dataset.df
...
def test_lean_boosted_lightgbm_classifier():
	# Regression
	lightgbm_classifier_cls = get_lightgbm_learner('lightgbm.LGBMClassifier')
	dataset = BankNote()
	target_column = dataset.y_column
	df = dataset.df
...
def test_lean_boosted_tensorflow_classifier():
	...
	tf_classifier_cls = get_tensorflow_dense_learner('KerasClassifier', layers, loss, optimizer=optimizer, \
		sk_params={'epochs': 100, 'batch_size': 100})
	dataset = BankNote()
	target_column = dataset.y_column
	df = dataset.df
...
def test_lean_boosted_pytorch_classifier():
	...
	# Regression
	layers = [(10, 'relu'), (5, 'relu'), (1, 'sigmoid')]
	pt_classifier_cls = get_pytorch_dense_learner('skorch.NeuralNetClassifier', layers, sk_params={'max_epochs': 100, 'batch_size': 100, 'criterion': nn.BCELoss})
	dataset = BankNote()
	target_column = dataset.y_column
	df = dataset.df
